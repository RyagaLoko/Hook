...
class CPFsCharacter
{
	...
	{	
		...
		//Detour(reinterpret_cast<PBYTE>(0x00402F80), reinterpret_cast<PBYTE>(MF(GetCharInventory)));
...
///There is a bug in the code provided. The issue is with the `Detour` function call. The `Detour` function is not a standard C++ function and it seems like it is being used incorrectly.
///If you are trying to hook the `fc_NC_CHAR_CHARDATA_REQ` function at address `0x00438DC0`, you need to use a proper hooking library or technique. The `Detour` function call in the code snippet is not a valid way to achieve function hooking.
///You should use a proper function hooking library like Microsoft Detours, MinHook, or other similar libraries to hook functions in a safe and reliable manner. Make sure to include the necessary headers and link the library properly before using the hooking functions provided by the library.
	}
	...
	HOOK_PUB_S bool usp_InstanceDungeon_GetTop3(CGameDB_Character* db, char mapIndx[12], BYTE modeId, BYTE type)
	{
		...
		auto v4 = SQLExecDirect(stmt, (SQLWCHAR*)L"{CALL usp_InstanceDungeon_GetTop3(?, ?, ?)}", SQL_NTS);
		return !v4 || v4 == 1;
	}
	...
	HOOK_PUB_S bool usp_InstanceDungeon_GetMyRank(CGameDB_Character* db, unsigned int charNo)
	{
		...
		auto v4 = SQLExecDirect(stmt, (SQLWCHAR*)L"{CALL usp_InstanceDungeon_GetByCharNo(?)}", SQL_NTS);
		return !v4 || v4 == 1;
	}
	...
	HOOK_PUB_S bool usp_InstanceDungeon_GetTop20(CGameDB_Character* db, char mapIndx[12], BYTE modeId, BYTE type)
	{
		...
		auto v4 = SQLExecDirect(stmt, (SQLWCHAR*)L"{CALL usp_InstanceDungeon_GetTop20(?, ?, ?)}", SQL_NTS);
		return !v4 || v4 == 1;
	}
	...
	HOOK_PUB_S bool usp_InstanceDungeon_SaveRank(CGameDB_Character* db, char mapIndx[12], BYTE modeId, uint playTime, SHINE_INDUN_RANK rank)
	{
		...
		auto v4 = SQLExecDirect(stmt, (SQLWCHAR*)L"{CALL usp_InstanceDungeon_SaveRank(?, ?, ?, ?, ?, ?, ?)}", SQL_NTS);
		return !v4 || v4 == 1;
	}
	...
	HOOK_PUB_S bool usp_InstanceDungeon_GetAllRanks(CGameDB_Character* db, uint charNo, char mapIndx[12], BYTE modeId, BYTE type)
	{
		...
		auto v4 = SQLExecDirect(stmt, (SQLWCHAR*)L"{CALL usp_InstanceDungeon_GetAllRanks(?, ?, ?, ?)}", SQL_NTS);
		return !v4 || v4 == 1;
	}
